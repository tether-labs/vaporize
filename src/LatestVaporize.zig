...
pub fn MarkerDown(self: *Self, components: anytype) type {
    return struct {
        self: *Self = self,
        node: ?*Node = null,
...
pub fn compileMarkdown(self: *Self, source: []const u8, tagged_functions: anytype) !*MarkerDown(@TypeOf(tagged_functions)) {
    ...
    const markdown = try self.allocator.create(MarkerDown(TaggedFunctionType));
    markdown.* = .{
        .node = node,
...
pub fn comptimeForm(self: *Self, comptime T: type) type {
    return struct {
        ...
        pub fn render(vaporize: *const Vaporize) void {
            vaporize.self.traverseForm(vaporize.self.root, void, null, vaporize.inner_form) catch |err| {
                Vapor.printErr("Error: {any}\n", .{err});
            };
        }
    };
...
pub inline fn traverseForm(self: *Self, node: ?*Node, comptime T: type, functions: ?[]const TaggedFunction(T), value: anytype) !void {
    var current = node;
        ...
        switch (n.tag) {
            .Form => {
                Form(validateForm, value).style(&.{
                    ...
                    try self.traverseFormValue(n.child, .{}, T, functions, value);
...
pub fn traverseFormValue(self: *Self, node: ?*Node, style: Style, comptime T: type, functions: ?[]const TaggedFunction(T), value: anytype) !void {
    var current = node;

    const code_style: Vapor.Types.Style = self.style_config.code_style;
    const text_style: Vapor.Types.Style = self.style_config.text_style;
    const heading_style: Vapor.Types.Style = self.style_config.heading_style;
    const button_style: Vapor.Types.Style = self.style_config.button_style;
    const list_style: Vapor.Types.Style = self.style_config.list_style;
    const struct_style: Vapor.Types.Style = self.style_config.struct_style;

    while (current) |n| {
        switch (n.tag) {
            .Root => {
                try self.traverse(n.child, style, T, functions);
            },
            .Svg => |_| {
                ...
                Box().style(&.{
                    ...
                    Graphic(.{ .src = src }).style(&.{
                        ...
                        .visual = .{ .fill = text_style.visual.?.text_color, .stroke = text_style.visual.?.text_color },
            ...
            .Box => {
                Box().style(&struct_style)({
                    try self.traverseFormValue(n.child, style, T, functions, value);
                });
            },
            .Button => {
            ...
            .Heading => |_| {
                Box().style(&.{
                    ...
                    try self.traverseFormValue(n.child, .{
            ...
            .TextField => {
                Box().layout(.left_center).direction(.column).width(.percent(100)).height(.fit).spacing(2).children({
                    Label(n.data.textfield.title).style(&text_style);
